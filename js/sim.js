...
export function fuelRates(units, types, cfg = defaultConfig) {
  const cost = {};
  const co2 = {};
  const gen = {}; // MW generated by each fuel group
  ...
  return { cost, co2, gen, costTotal: sum(cost), co2Total: sum(co2) };
}
...
export function step(state, world, cfg = defaultConfig) {
  ...
  if (s.units.some((u) => u.auto)) {
    ...
    s = { ...s, units: autoDispatch(s.units, world.types, needMW, cfg, minute) };
  }
